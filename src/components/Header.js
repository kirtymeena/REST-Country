...
import { themify } from "../redux/features/themeSlice";
...
const Header = ({ theme }) => {
  ...
  useEffect(() => {
    dispatch(themify(isDark))
  });


  return (
    <header
      className={`nav__header container ${isDark ? "dark_nav" : "light_nav"} `}
    >
      <div>
      ...
      <div className="nav__icons">
        ...
        <div>
          {isDark ? (
            <h3 className="nav__text">Light Mode</h3>
          ) : (
            <h3 className="nav__text">Dark Mode</h3>
          )}
        </div>
      </div>
    </header>
